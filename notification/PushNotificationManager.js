    ...
    if (Platform.OS === 'android') {
        try {
            ...
            if (Notifications && Notifications.createChannel) {
                ...
                console.warn("Módulo nativo de notificaciones no disponible, usando canal por defecto de Firebase");
            }
        } catch (error) {
...
export async function registerForPushNotificationsAsync() {
    ...
    try {
        ...
        console.log("Firebase Messaging inicializado en el frontend:", firebaseApp.name);
    } catch (error) {
    ...
    if (Platform.OS === 'android' && Platform.Version >= 33) {
        try {
            ...
            const granted = await PermissionsAndroid.request(
                PermissionsAndroid.PERMISSIONS.POST_NOTIFICATIONS,
                {
                    ...
                    message: 'Esta aplicación necesita permiso para enviarte notificaciones.',
        ...
        console.log("No se requiere solicitud de POST_NOTIFICATIONS (API < 33 o no Android)");
    }
    ...
    try {
        ...
        if (enabled) {
            console.log("Permisos de Firebase otorgados, intentando obtener token...");
...
export function setupNotificationListener(listener) {
    ...
    const unsubscribeForeground = messaging().onMessage(async (remoteMessage) => {
        console.log('Notificación recibida en foreground:', JSON.stringify(remoteMessage));
        listener(remoteMessage);
    });

    messaging().setBackgroundMessageHandler(async (remoteMessage) => {
        console.log('Notificación recibida en background:', JSON.stringify(remoteMessage));
    });

    messaging().onNotificationOpenedApp((remoteMessage) => {
        console.log('App abierta desde notificación:', JSON.stringify(remoteMessage));
    });
    ...
    messaging()
        ...
        .then((remoteMessage) => {
            if (remoteMessage) {
                console.log('App iniciada desde notificación:', JSON.stringify(remoteMessage));
            }
        });
